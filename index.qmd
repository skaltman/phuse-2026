...
cholesterol <- round(120 + 2.6 * bmi + rnorm(n, 0, 18), 1)

imp_bmi <- round(runif(n_imp, 20, 40), 1)
imp_chol <- round(120 + 2.6 * imp_bmi, 1)

bmi_data <- data.frame(
  bmi = c(bmi, imp_bmi),
  cholesterol = c(cholesterol, imp_chol)
)

ggplot(bmi_data, aes(x = bmi, y = cholesterol)) +
...
<img src="figures/posit-assistant-pause-often.png" alt="A screenshot of the Posit Assistant panel mid-analysis. The assistant has plotted the top 20 states by number of Spanish speakers, summarized the result in a short paragraph noting that raw counts aren't normalized by population, and paused to suggest three follow-up questions rather than continuing on its own." style="height: 580px; display: block;">
</div>
</div>
